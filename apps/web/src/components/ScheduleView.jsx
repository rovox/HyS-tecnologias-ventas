...
const ScheduleView = ({ types = [], title }) => {
  const { currentUser } = useAuth();
  const { vendors } = useVendedorList();
  ...
  return (
    <div className="space-y-6 flex flex-col min-h-[calc(100vh-120px)] lg:h-[calc(100vh-120px)] relative w-full max-w-full overflow-x-hidden">
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 shrink-0 w-full">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground break-words w-full">{title}</h2>
          <p className="text-muted-foreground font-medium mt-1 capitalize">{getDateDisplay()}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full xl:w-auto">
          ...
          <Button className="ml-auto xl:ml-2 gap-2 bg-primary text-primary-foreground font-bold shadow-md hover:bg-primary/90 hover:-translate-y-0.5 transition-all" onClick={handleOpenCreate}>
            <Plus className="h-4 w-4" /> Nuevo Trabajo
          </Button>
        </div>
      </div>

      <div className="flex-1 min-h-0 w-full relative bg-card border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
        {loading ? (
        ...
        ) : schedulesInView.length === 0 && viewMode === 'weekly' ? (
